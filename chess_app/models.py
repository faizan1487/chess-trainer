...
class Opening(models.Model):
    ...
    )  # e.g., "Open Game", "Semi-Open", etc.
    main_line = models.TextField(
        blank=True, null=True
    )  # Optional field for the main line PGN
    parent_opening = models.ForeignKey(
...
class Game(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=GAME_STATUS_CHOICES, default='ONGOING'
    )
    ...
    in_opening_book = models.BooleanField(
        default=True
    )  # Whether we're still in the opening book
...
class Move(models.Model):
    ...
    quality = models.CharField(
        max_length=10, choices=MOVE_QUALITY_CHOICES, default='normal'
    )
...
class UserProgress(models.Model):
    ...
    avg_accuracy = models.FloatField(
        default=0.0
    )  # Average move accuracy 0-100
    best_accuracy = models.FloatField(
        default=0.0
    )  # Best game accuracy 0-100
...
class Challenge(models.Model):
    ...
    opening = models.ForeignKey(
        Opening, on_delete=models.CASCADE, related_name='challenges'
    )
